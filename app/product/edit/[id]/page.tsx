...
const toDraft = (raw: any): Draft => {
  ...
  const ensureTier = (t: any): TierForm => ({
    ...
    specials:
      typeof t?.specials === "object" && t?.specials !== null ? t.specials : {},
  });

  const images = Array.isArray(raw?.media?.images)
    ? (raw.media.images.filter(Boolean) as string[])
    : [];
  const publicIds = Array.isArray(raw?.media?.publicIds)
    ? (raw.media.publicIds.filter(Boolean) as string[])
    : undefined;
  ...
  return {
    ...
    subServices: Array.isArray(raw?.subServices)
      ? (raw.subServices.filter(Boolean) as string[])
      : [],
    tiers: {
...
export default function EditProductPage() {
  ...
  const stepItems: StepMeta[] = useMemo(() => {
    ...
    return (labels as string[]).map((label, idx) => {
  ...
  if (forbidden) {
    return (
      <main className="grid min-h-[calc(100vh-64px)] place-items-center text-sm text-gray-500">
        Kamu tidak memiliki akses untuk mengedit produk ini.
      </main>
    );
  }
