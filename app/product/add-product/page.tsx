...
import { ensureBucketId, listPendingFiles, clearAll, clearBucketByKey } from "@/app/lib/deferredMedia";
...
export default function AddProductPage() {
  ...
  const BUCKET_KEY = useMemo(() => `add-product:${globalThis.crypto?.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(36).slice(2)}`}`, []);
  ...
  const onSubmit = async () => {
    ...
    try {
      ...
      if (draft.media.pdfUrl && draft.media.pdfUrl.startsWith("data:application/pdf")) {
        const r = await uploadDataUrlToCloudinary(draft.media.pdfUrl, "document.pdf", "products/docs", "raw");
    ...
    <main className="mx-auto max-w-3xl p-4">
      ...
      <div className="mb-6 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <button aria-label="Kembali" onClick={goBackStep} className="inline-flex h-9 w-9 items-center justify-center rounded-full border border-gray-300 hover:bg-gray-100" title="Kembali">
      ...
      {step === 3 && (
        <Step3Media
          ...
          bucketKey={BUCKET_KEY} // bucket unik per sesi
        />
      )}
      ...
      <div className="mt-6 flex items-center justify-end gap-2">
        ...
        {step < 4 ? (
          ...
          <button type="button" disabled={saving} onClick={onSubmit} className="rounded-xl bg-gray-900 px-4 py-2 text-white hover:bg-gray-800 disabled:opacity-50">
